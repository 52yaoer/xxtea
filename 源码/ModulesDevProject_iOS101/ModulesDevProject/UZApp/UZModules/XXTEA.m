...
+ (NSString *) encryptToHexString:(NSData *)data stringKey:(NSString *)key {
    return [self convertDataToHexStr: [self encrypt:data stringKey:key]];
}
...
+ (NSString *) encryptStringToHexString:(NSString *)data stringKey:(NSString *)key {
    return [self encryptToHexString:[data dataUsingEncoding:NSUTF8StringEncoding] stringKey:key];
}
...
+ (NSString *) decryptHexStringToString:(NSString *)data key:(NSData *)key {
    NSData * tmp = [self convertHexStrToData:data];
    if (tmp == nil) return nil;
    return [self decryptToString:tmp key:key];
}
...
+ (NSString *) decryptHexStringToString:(NSString *)data stringKey:(NSString *)key {
    return [self decryptHexStringToString:data key:[key dataUsingEncoding:NSUTF8StringEncoding]];
}
